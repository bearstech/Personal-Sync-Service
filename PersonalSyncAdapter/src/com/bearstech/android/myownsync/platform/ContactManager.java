...
public class ContactManager {
     ...
     */
    public static final String CUSTOM_IM_PROTOCOL = "MyOwnSync";
    ...
    public static synchronized void syncContacts(Context context,
        ...
        Log.d(TAG, "In SyncContacts for "+users.toString());
        for (final User user : users) {
        	Log.d(TAG, "*** using user: "+user);
    ...
    private static void addContact(Context context, String accountName,
        ...
        Log.d(TAG,"add contact name "+user.getLastName());
        contactOp.addUser(user).addEmail(user.getEmails())
                               .addNickname(user.getNickname())
        					   .addPhone(user.getPhones())
        					   .addAddresses(user.getAddresses())
    ...
    private static void updateContact(Context context,
    	...
    	Log.d(TAG,"update contact "+user.getLastName());
    	Uri uri;
    	String phone = null;
    	String email = null;
    	...
    	try {
    		while (c.moveToNext()) {
    			...
    			if (mimeType.equals(StructuredName.CONTENT_ITEM_TYPE)) {
    				...
    				phone = c.getString(DataQuery.COLUMN_PHONE_NUMBER);
    				contactOp.updatePhones(phone, user.getPhones(),uri);
    			}

    			else if (Data.MIMETYPE.equals(Email.CONTENT_ITEM_TYPE)) {
    				email = c.getString(DataQuery.COLUMN_EMAIL_ADDRESS);
    				contactOp.updateEmails(user.getEmails(), email, uri);

    			}

    			else if (Data.MIMETYPE.equals(Nickname.CONTENT_ITEM_TYPE)) {
    				nickname = c.getString(DataQuery.COLUMN_DATA1);
    				contactOp.updateNickname(user.getNickname(), nickname, uri);
    			}
        ...
        if (phone == null) {
            contactOp.addPhone(user.getPhones());
        }

        // Add the email addresses, if present and not updated above
        if (email == null) {
            contactOp.addEmail(user.getEmails());
        }
